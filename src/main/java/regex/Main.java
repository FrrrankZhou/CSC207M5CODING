...
public class Main {
    ...
    public static boolean checkForPassword(String str, int minLength) {
        ...
        final boolean propertyLength = str.length() >= minLength;
        final boolean propertyLower = Pattern.matches(".*[a-z].*", str);
        final boolean propertyUpper = Pattern.matches(".*[A-Z].*", str);
        final boolean propertyDigit = Pattern.matches(".*[0-9].*", str);

        return propertyLength && propertyLower && propertyUpper && propertyDigit;
    }
